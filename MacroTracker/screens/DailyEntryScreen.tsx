// DailyEntryScreen.tsx (Corrected and Fully Functional)
...
const DailyEntryScreen: React.FC = () => {
    ...
    const getCurrentEntry = (): DailyEntry => {
        const existingEntry = dailyEntries.find((entry) => entry.date === selectedDate);
        if (existingEntry) {
            return existingEntry;
        } else {
            // Create a new DailyEntry if one doesn't exist
            return {
                date: selectedDate,
                items: [],
            };
    ...
    const handleAddEntry = async () => {
        ...
        const currentEntry = getCurrentEntry();  // Get or *create* the entry
        let updatedItems;
        ...
        if (editIndex !== null) {
            ...
            updatedItems = currentEntry.items.map((item, index) =>
             ...
             // Add the new item to the (potentially new) entry
        ...
        const updatedEntry = { ...currentEntry, items: updatedItems };
         ...
         // Find and replace OR add the new entry
        let updatedEntries = dailyEntries.map((entry) => {
        ...
        if (!dailyEntries.some(entry => entry.date === selectedDate)) {
            updatedEntries.push(updatedEntry);
        }
