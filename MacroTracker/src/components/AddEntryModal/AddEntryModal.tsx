...
const AddEntryModal: React.FC<AddEntryModalProps> = ({
  ...
  // Main initialization effect for modal visibility and form state
  useEffect(() => {
    if (isVisible) {
  ...
  // Effect specifically for requesting icon for the food item being edited OR a pre-selected new food
  useEffect(() => {
      ...
      try {
        ...
        if (pickerResult.assets && pickerResult.assets.length > 0) {
          ...
          if (results.length === 0) {
            ...
            setQuickAddLoading(false); // Loading is done
            setModalMode("normal"); // No items, go back to normal
          } else {
            ...
            setQuickAddLoading(false); // Set loading to false immediately after items are set
            // Modal mode is already 'quickAddSelect'
          }
