...
// --- Function for Multiple Foods (Using the imported getChatCompletion) ---
/**
...
export async function getMultipleFoodsFromImage(asset: {
  ...
  const prompt = `
    ...
    2. An 'estimatedWeightGrams' based on visual approximation (be reasonable, e.g., between 10 and 1000).
    3. The estimated nutritional content per 100 grams: 'calories_per_100g', 'protein_per_100g', 'carbs_per_100g', 'fat_per_100g'. If the image clearly shows a nutrition label with per 100g values, prioritize using those values. Otherwise, provide your best estimate based on the identified food type. Ensure all nutritional values are non-negative numbers.
  ...
  try {
    base64File = await getBase64FromUri(asset.uri);
    console.log("Successfully converted multi-food image to base64.");
  } catch (err) {
    console.error("Error getting base64 from URI in getMultipleFoodsFromImage:", err);
    throw new Error("Failed to read image file."); // Re-throw for modal to catch
  }
  ...
  const imageUrl = `data:${mimeType};base64,${base64File}`;
  console.log(`Determined MIME type (multi-food): ${mimeType}`);
  ...
  try {
    ...
    // *** Uses the imported getChatCompletion ***
    const response = await getChatCompletion(
        "google/gemini-2.0-flash-thinking-exp-1219:free", // Ensure this model supports vision and JSON output
        messages,
        "json_object" // Request json_object, model should handle array within it
        // API key handled within getChatCompletion
    );

    const rawContent = response?.choices?.[0]?.message?.content;
    console.log("Raw API Response Content (Multi-Food):", rawContent);

    if (!rawContent || typeof rawContent !== 'string') {
      console.error("Invalid or non-string response content from AI (multi-food):", rawContent);
      throw new Error("Invalid or non-string response content from AI.");
    }

    const cleanedContent = rawContent.trim().replace(/^```json\s*|\s*```$/g, "");
    console.log("Cleaned JSON Content (Multi-Food):", cleanedContent);

    // Check for empty array explicitly before parsing
    if (cleanedContent === '[]') {
        console.log("AI identified no food items, returning empty array.");
        return [];
    }

    // 5. Parse and Validate JSON Array
    let estimatedItems: any[];
    try {
        estimatedItems = JSON.parse(cleanedContent);
    } catch (parseError) {
        if (parseError instanceof SyntaxError) {
            console.error("Error: The multi-food response content was not valid JSON.", parseError);
            console.error("Cleaned content that failed parsing:", cleanedContent);
            throw new Error("The AI response for multiple foods could not be understood. Please try again or add items manually.");
        }
    ...
    const validatedItems: EstimatedFoodItem[] = estimatedItems.map((item, index) => {
      if (
        ...
        console.error(`Parsed JSON array item at index ${index} has incorrect structure or empty name:`, item);
        throw new Error(`AI returned an array item with unexpected format or empty name at index ${index}.`);
      }

      // Ensure non-negative and reasonable values
      const validatedWeight = Math.max(1, Math.min(5000, Math.round(item.estimatedWeightGrams)));
      const validatedCalories = Math.max(0, Math.round(item.calories_per_100g));
      const validatedProtein = Math.max(0, Math.round(item.protein_per_100g));
      const validatedCarbs = Math.max(0, Math.round(item.carbs_per_100g));
      const validatedFat = Math.max(0, Math.round(item.fat_per_100g));

      if (validatedWeight !== item.estimatedWeightGrams || validatedCalories !== item.calories_per_100g || validatedProtein !== item.protein_per_100g || validatedCarbs !== item.carbs_per_100g || validatedFat !== item.fat_per_100g) {
        console.warn(`Adjusted values for item "${item.foodName}" at index ${index}. Original:`, item, `Adjusted: { weight: ${validatedWeight}, cal: ${validatedCalories}, p: ${validatedProtein}, c: ${validatedCarbs}, f: ${validatedFat} }`);
      }

      return {
        foodName: item.foodName.trim(),
        estimatedWeightGrams: validatedWeight,
        calories_per_100g: validatedCalories,
        protein_per_100g: validatedProtein,
        carbs_per_100g: validatedCarbs,
        fat_per_100g: validatedFat,
      };
    });

    console.log(`Successfully extracted ${validatedItems.length} food item(s) from multi-food image.`);
    return validatedItems;

  } catch (error) {
    console.error("Error in getMultipleFoodsFromImage:", error);
    if (error instanceof Error && (error.message.includes("could not be understood") || error.message.includes("AI returned data that is not a JSON array") || error.message.includes("unexpected format") || error.message.includes("Invalid or non-string"))) {
       throw error; // Re-throw specific, potentially user-friendly errors
    }
    // Throw generic error for other issues (like API call failures)
    throw new Error(
      `Failed to get multiple foods from image: ${ error instanceof Error ? error.message : "An unknown error occurred"}`
    );
  }
}
